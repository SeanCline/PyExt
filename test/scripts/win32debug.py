"""Wrappers around various Win32 APIs debugging.
Allows writing dump files, triggering debug breaks, detecting  whether a
debugger is attached and printing messages to the debugger.
"""

import os, sys, ctypes, msvcrt, threading
...
def _mini_dump_write_dump(dump_file_name, process_id, dump_type):
...
def _launch_and_wait(target, args=()):
    """Runs a function on another thread, waits for it to finish, and returns the result or raises."""
    def closure():
        try:
            closure.result = target(*args)
        except Exception as ex:
            closure.exception = ex
    
    closure.result = closure.exception = None
    t = threading.Thread(target=closure)
    t.start()
    t.join()
    
    if closure.exception != None:
        raise closure.exception
...
def _generate_default_dump_filename():
    ...
    return '.'.join([base_filename_str, python_ver_str, bittess_str, "dmp"])


def dump_process(dump_file_name=None, process_id=None):
    """Writes a dump of a given pid (or the current process if not specified) to a file."""
    
    # Default args.
    process_id = process_id or ctypes.windll.kernel32.GetCurrentProcessId()
    dump_file_name = dump_file_name or _generate_default_dump_filename()
    ...
    _launch_and_wait(_mini_dump_write_dump, (dump_file_name, process_id, dump_type))
